...
function initMap() {                     // method to initialise the map and define constants
  
  defaultZoom = $(window).width() < 500 ? 11 : 12;
  map = new google.maps.Map(document.getElementById('map'), {  
      // Code Source --- https://developers.google.com/maps/documentation/javascript/examples/control-default
    zoom: defaultZoom,                // var defaultZoom -- defined above ...
    center: sfCenter,                 // SF LaT LonG given above 
    disableDefaultUI: true,  //Turned OFF Default_UI-https://developers.google.com/maps/documentation/javascript/controls          
    zoomControl: true, 
    zoomControlOptions: {
  ...
  imgWidth = $(window).width() < 500 ? 80 : 115; //jQuery - Img Width to be adjusted when Windows Width is Less than 500px
}
...
function searchListener (title) {
  ...
  if (title == '') {
    ...
    // Two API's used to access Movies Data - SF Gov and  "the MOVIE DB" 
  ...
  var get1 = $.get(api_url);         // AJAX call using jQuery to Access the API's 
  var get2 = $.get(pic_api_url);     // AJAX call using jQuery to Access the API's 
...
function messageBox (title, locations) {
  ...
  if (title == '') {
    ...
    if (titles.indexOf(title) >= 0) {
      if (title in markers) {                            // If movie name chosen already exists in MARKERS 
        error = true;
